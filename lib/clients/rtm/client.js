...
function hasExceededRtmStartLimit() {
  ...
  rtmStartAttempts = rtmStartAttempts.filter(function (item) {
    return item > oneHourAgo;
  });

  return rtmStartAttempts.length > MAX_RTM_START_PER_HOUR;
}
...
RTMClient.prototype.start = function start(opts) {
  ...
  if (!this._connecting) {
    ...
    // DDoS Check
    if (hasExceededRtmStartLimit()) {
      var msg = 'Exceeded RTM connection limit in the last hour (' + MAX_RTM_START_PER_HOUR + ')';
      this.disconnect(msg, new Error(msg));
      return;
    } else {
      rtmStartAttempts.push(Date.now());
    }
